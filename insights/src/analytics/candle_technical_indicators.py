...
def upsert_candle_queue(candle, store):
    queue_key = generate_store_key(candle.product_id, candle.granularity_mins)
    if queue_key not in store:
        store[queue_key] = SortedList(key=lambda candle: candle.start)

    candle_queue = store[queue_key]
...
def update_technical_indicators(latest_candle_data, store):
    upsert_candle_queue(latest_candle_data, store)
    inputs = get_ohlcv_inputs(latest_candle_data.product_id, latest_candle_data.granularity_mins, store)
    ...
    row_index = None
    try:
        row_index = technical_indicators["time"].index(latest_candle_data.start)
    except ValueError:
        _logger.warn(f"Could not find candle time {latest_candle_data.start} in indicator time array")

    if row_index is not None:
        for key, values in technical_indicators.items():
            if isinstance(values, list) and len(values) > row_index:
                latest_indicator_values[key] = values[row_index]

    latest_indicator_values["time"] = latest_candle_data.start
    latest_indicator_values["id"] = latest_candle_data.product_id
    latest_indicator_values["granularity_mins"] = latest_candle_data.granularity_mins
    return latest_indicator_values

def update_technical_indicators_batch(batch_candle_data, store):
    sorted_batch_data = sorted(batch_candle_data, key=lambda x: x.start)
    analysis_batch = []
    for latest_candle_data in sorted_batch_data:
        latest_analysis = update_technical_indicators(latest_candle_data, store)
        print(f"TIIME: {latest_analysis['time']}, OOPEN: {latest_analysis['open']}")
...
def get_ohlcv_inputs(product_id, granularity_mins, store):
    store_key = generate_store_key(product_id, granularity_mins)
    if store_key not in store:
        return {}
    candle_queue = store[store_key]
    if not candle_queue:
        return {}
    return {
        'id': numpy.array([candle.product_id for candle in candle_queue]),
