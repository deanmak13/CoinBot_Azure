...
# Global buffer for events: keys are event IDs, values are event dicts with a 'received_at' timestamp.
buffer_store = {}
...
def handle_events():
    try:
        ...
        _logger.error(f"Error handling event: {e}")
...
def process_event(events):
    ...
    for event in events:
        try:
            ...
            _logger.error(f"Invalid event id: {event.get('id')}")
            continue
...
def flush_buffer(now):
    ...
    for event_id in sorted_ids:
        event = buffer_store[event_id]
        if now - event['received_at'] >= DELAY_THRESHOLD:
            _logger.info(f"Flushing Event I.D: {event_id} from buffer store, time threshold met")
    ...
    if len(buffer_store) >= BUFFER_SIZE_THRESHOLD:
        ...
        event = buffer_store[min_event_id]
        _logger.info(f"Flushing Event I.D: {min_event_id} from buffer store, size threshold met")
        process_ordered_event(event)
